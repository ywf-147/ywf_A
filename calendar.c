...
//原来的文件名：C51写的公历转农历和星期（杨注：可以一试，好像以前用的就是这个）
/*
C51写的公历转农历和星期 
...
公历年对应的农历数据,每年三字节,
格式第一字节u87-4 位表示闰月月份,值为0 为无闰月,u83-0 对应农历第1-4 月的大小
第二字节u87-0 对应农历第5-12 月大小,第三字节u87 表示农历第13 个月大小
月份对应的位为1 表示本农历月大(30 天),为0 表示小(29 天)
第三字节u86-5 表示春节的公历月份,u84-0 表示春节的公历日期
*/
u8 year_code[597]={
...
///月份数据表
u8 day_code1[9]={0x0,0x1f,0x3b,0x5a,0x78,0x97,0xb5,0xd4,0xf3};
u16 day_code2[3]={0x111,0x130,0x14e};
/*
...
void Conver_week(u8 c,u8 year,u8 month,u8 day)
{//c=0 为21世纪,c=1 为19世纪 输入输出数据均为BCD数据
    u8 temp1,temp2;
    temp1=year/16;   //BCD->hex 先把数据转换为十六进制
    temp2=year%16;
...
/*test
u8 c_sun,year_sun,month_sun,day_sun;
void main(){
c_sun=1;
year_sun=0x2;
month_sun=0x11;
